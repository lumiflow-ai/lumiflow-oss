...
import { SignupResponseSchema } from "./definitions";

describe("Signup Route", () => {
  it("post /v0.1/signup creates a new org and updates the user", async () => {
    await fakePersistence(async (pgPool) => {
      ...
      const response = await supertest(app)
        .post("/v0.1/signup")
        .send({ org: { name: "Test" }, user: { fullName: "New Name" } })
        .expect(200);

      const orgResponse = SignupResponseSchema.parse(response.body);
      ...
      expect(updatedUser?.fullName).to.equal("New Name");
  ...
  it("post /v0.1/signup returns unauthorized when no auth is present", async () => {
    await fakePersistence(async (pgPool) => {
      const app = createApp({
